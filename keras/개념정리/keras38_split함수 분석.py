...
# for 변수 in range(종료값): # range()이건 함수임. range()함수는 입력 받은 숫자에 해당되는 범위의 값을 반복 가능한 객체로 만들어 출력해줌. 
# 쉽게 말해서 range()함수를 이용하면 특정 구간의 정수들을 생성할 수 있음
...
def split_x(dataset, size):                        # 함수선언 + 데이터,timestep 값 입력
    aaa = []                                       # aaa라는 리스트 선언
    for i in range(len(dataset) - size + 1):       # 데이터셋(a)의 길이 - timesteps값 + 1만큼의 횟수만큼 반복하겠다 (결국 range(6)이니까 0~5 까지를 i에 넣어서 순차적으로 반복 시켜주겠다~)       
        subset = dataset[i : (i + size)]           # subset이 뭐냐면 -> 로드한 dataset을 슬라이싱해서 그 부분만큼만 불러옴 리스트형태로. -> 그걸 subset에 저장
        aaa.append(subset)                         # aaa라는 리스트에 subset을 추가하겠다 (aaa와 subset을 합쳐주겠다)    
...
# 행과 열을 각각 슬라이싱해줘야 한다. ,로 구분하고 :만 쓰는건 전체를 다 쓰겠다는 뜻
