...
model = Sequential()
# model.add(Dense(units=10, input_shape=(5, 5, 1))) <- 이렇게 해줘도 아래(conv2d)와 같음
...
model.add(Conv2D(filters=10, kernel_size=(2,2), # 출력 (N, 4, 4, 10) # kernel_size=(이미지를 자르는 규격) 
                 input_shape=(5, 5, 1))) # (N, 5, 5, 1)  # (batch_size, rows, colums, channels) -> 이미지 개수, 행, 열, 흑백 or 컬러
# 다음 레이어의 input_shape으로 전달 될 때는 (n, 4, 4, 10) -> (N, (5-2)+1/1, (5-2)+1/1, 직전 레이어의 필터값)
# 필터 값이 다음 레이어의 채널 값이 됨

# model.summary()  # (kernel_size * channels + bias) * filters = summary Param 갯수 (CNN 마들)

model.add(Conv2D(7, (2,2), activation='relu')) # 출력 (N, 3, 3, 7)
# CNN 모델의 출력은 (10, ) or (10, 1) 이러한 2차원 형태로 출력됨 
# 
# 따라서 Dense 형태로 바꿔줘야 함 (4차원의 형태를 2차원의 형태로 바꿔줘야 함)
# 어느정도 작업을 하다가 중간에 쫙 펴줌 (reshape 개념) 그 뒤로 Dense 모델로 진행
model.add(Flatten()) # <- 쫙 펴주기 위해선 이놈을 써줘야 함  (N, 63) 그래서 이렇게 바뀜
